...
A modern, typed, and cacheable TypeScript client for
the [Porkbun v3 API](https://porkbun.com/api/json/v3/documentation).
...
* **Flexible Caching:** Uses [Keyv](https://github.com/jaredwray/keyv) to support any cache backend (in-memory, Redis,
  SQLite, etc.).
...
apiKey: process.env.PORKBUN\_API\_KEY\!,  
secretApiKey: process.env.PORKBUN\_SECRET\_API\_KEY\!,  
});
...
This library uses [Keyv](https://github.com/jaredwray/keyv) for caching. You can pass *any* Keyv-compatible store to the
client constructor.
...
Creates a new DNS record. This method is **not** cached and will **automatically invalidate** the getAllRecords cache
for this domain.
...
name: 'www', // The subdomain (e.g., 'www' for \[www.example.com\](https://www.example.com))  
type: 'A',  
content: '1.2.3.4',  
...
If the Porkbun API returns an error (e.g., "Invalid API Key" or "Domain not found"), the client will throw a
PorkbunApiError.
...
[MIT](http://docs.google.com/LICENSE)